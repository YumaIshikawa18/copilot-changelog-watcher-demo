...
GitHub Changelog の Copilot 更新を最新 50 件まで定期取得し、AI 要約やメール送信を使わずに、キーワードベースで `high / medium / low` に分類した静的ダッシュボードを GitHub Pages へ公開するサンプル実装です。

## 概要

- 監視対象: Copilot RSS と Copilot Changelog 一覧ページ
- 取得件数: 最新 50 件
- 分類方法: 記事タイトル、RSS 要約、Changelog 種別・タグに含まれるキーワードで `high / medium / low` を付与
...
1. GitHub Changelog の Copilot RSS と Changelog 一覧ページを取得します。
2. 一覧ページをたどって最新 50 件を集めます。
3. 記事タイトル、RSS 要約、種別、タグからキーワードを検出し、重要度を分類します。
4. 静的サイト用の `dist/data.json` を生成します。
5. `site/` 配下のフロント資産と合わせて GitHub Pages へデプロイします。
...
- この実装は RSS 要約と Changelog 一覧のメタ情報だけを材料に分類するため、厳密な重要度判定ではなく軽量な閲覧支援です
